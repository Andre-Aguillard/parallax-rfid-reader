...
def RFID_tag():
    ...
    # Gives the user 15 seconds to scan his/ her card, this is more than enough time to get a reading. 
    while (int(time()*1000) < startTime + 15 *1000):
        response = "Entering RFID mode. \n\nPlease scan your RFID card now."
        display(response)
        ###Note: The following is commented out because it doesn't work without an RIFD reader 
        """data = ser.read(12)
...
def inputNumber(number):
        ...
        if (len(ManCode)== 10):
            ...
            if (number==('#')):
                if(len(ManCode)<10):
                    response = "Pound sign (#) should only be pressed at the end of your code, \nplease input a different number"
                    display(response)
                    window.update
                    sleep(3)
                else:
